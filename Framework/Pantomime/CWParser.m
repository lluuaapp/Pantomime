...
{  
  ...
  if (theBOOL)
	...
	case PantomimeResentToRecipient:
      ...
      //NSLog(@"The Line = |%@|", [theLine asciiString]);
      aData = [theLine subdataFromIndex: len];
    }

  // CAN'T WORK with something like: "Hubbard, David" <dhubbard@dino.hostasaurus.com>
  bytes = (unsigned char*)[aData bytes];
  len = [aData length];
  b = NO; x = 0;
		      ...
		      quick: (BOOL) theBOOL
{
  ...
  else aData = [theLine subdataFromIndex: 13];
  ...
  index = [aData indexOfCharacter: ';'];

  if (index > 0) aData = [aData subdataToIndex: index];
...
// #warning FIXME - consider format= when passing the parameter range
  ...
  if (is_rfc2231)
      ...
      if (has_charset)
	{
	  ...
	  r2 = [aMutableData rangeOfCString: "'"  options: 0  range: NSMakeRange(NSMaxRange(r1), [aMutableData length]-NSMaxRange(r1))];
	  ...
	  [aMutableData replaceBytesInRange: NSMakeRange(0, NSMaxRange(r2))  withBytes: NULL  length: 0];
